...
const Grateful = () => {
  const { currentUser, handleGrateful, journalEntry } = useAuth();
  ...
  const createGratefulBlocks = journalEntry.grateful.map((item, index) => {
    return (
      <label key={currentUser.uid + index} className="label flex flex-row items-center justify-start">
        ...
        <input id={index} type="text" placeholder="..." className="input input-ghost text-neutral w-full max-w-full font-light placeholder:text-xl placeholder:lg:text-2xl text-xl lg:text-2xl focus:outline-none" value={journalEntry.grateful[index]} onChange={(e) => {
